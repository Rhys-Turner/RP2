# -*- coding: utf-8 -*-
"""
Created on Sat Oct  7 11:07:00 2023
...
## hierarical cluster labels

def read_csv_folder(folder_path):
    dfs = {}
    for filename in os.listdir(folder_path):       # int through the filess
...
## removed all sinlge vales from dict 
...
def split_dict_by_flux(dict_of_dfs):
    ...
    for key, df in dict_of_dfs.items():
        ...
        filtered_dict_n = df[df["flux"].between(-1000, 0)]  ## filter dictionary splitting the flux change the flux value here depending on the dataset
...
## must remove the singles a second time after the split of the datafarmes 
...
## inverting the negitive dictionary so that the neg-graphs resemble the same "side" of the flux graph 
...
## join dicts with neg keys changed 
pos_neg_dict = join_dicts(pos_dict, inv_neg_dict)


## z-normalization however missing entries where all the values are the same
...
## converting the dictionary of dataframes to list of arrays for the Dtw distance matrix 
...
## this must be the same as the input for the next function 
ds = dtwdistance_matrix(ts_full)


## clustering function to retrieve cluster lables 

def hier_clust_with_dtw(data_dict, ds_matrix, link, num_clust):
    keys = list(data_dict.keys())
    
    heir_clust = AgglomerativeClustering(num_clust, affinity='precomputed', linkage=link)  ## running the heir clust on the data with precomputed distance matrix and linkage
    
    cluster_labels = heir_clust.fit_predict(ds_matrix) 
...
## change depending on parameters 
clusters = hier_clust_with_dtw(ts_full, ds, "average",10)
## gives the clusters for plotting in a dict of dicts 